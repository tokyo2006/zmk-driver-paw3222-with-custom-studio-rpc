...
uint32_t paw3222_request_get_source(const xinta_paw3222_Request *req) {
    ...
    case xinta_paw3222_Request_get_info_tag:
        return req->request_type.get_info.source;
    case xinta_paw3222_Request_read_diagnostics_tag:
        return req->request_type.read_diagnostics.source;
    case xinta_paw3222_Request_read_register_tag:
        return req->request_type.read_register.source;
    case xinta_paw3222_Request_write_register_tag:
...
static void set_error(xinta_paw3222_Response *resp, const char *msg) {
...
static void handle_get_info(xinta_paw3222_Response *resp) {
    xinta_paw3222_GetInfoResponse *out = &resp->response_type.get_info;
    ...
    for (size_t i = 0; i < count && i < ARRAY_SIZE(out->devices); i++) {
        ...
        xinta_paw3222_DeviceInfo *info = &out->devices[out->devices_count++];
...
static void handle_read_diagnostics(const xinta_paw3222_ReadDiagnosticsRequest *req,
    ...
    xinta_paw3222_ReadDiagnosticsResponse *out = &resp->response_type.read_diagnostics;
    out->product_id1 = diag.product_id1;
    out->product_id2 = diag.product_id2;
    out->motion = diag.motion;
    out->cpi = diag.cpi;
    resp->which_response_type = xinta_paw3222_Response_read_diagnostics_tag;
}

static void handle_read_register(const xinta_paw3222_ReadRegisterRequest *req,
                                 xinta_paw3222_Response *resp) {
    const struct device *dev = paw3222_get_device(req->device_index);
    if (!dev) {
        set_error(resp, "invalid device_index");
    ...
    resp->which_response_type = xinta_paw3222_Response_read_register_tag;
}

static void handle_write_register(const xinta_paw3222_WriteRegisterRequest *req,
                                  xinta_paw3222_Response *resp) {
    const struct device *dev = paw3222_get_device(req->device_index);
    if (!dev) {
        set_error(resp, "invalid device_index");
    ...
    resp->which_response_type = xinta_paw3222_Response_write_register_tag;
}

bool paw3222_request_exec_handle(const xinta_paw3222_Request *req, xinta_paw3222_Response *resp) {
    switch (req->which_request_type) {
    case xinta_paw3222_Request_get_info_tag:
