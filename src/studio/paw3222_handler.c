...
static struct zmk_rpc_custom_subsystem_meta paw3222_feature_meta = {
    ZMK_RPC_CUSTOM_SUBSYSTEM_UI_URLS(
        "http://cormoran.github.io/zmk-driver-paw3222-with-custom-studio-rpc/"),
...
ZMK_RPC_CUSTOM_SUBSYSTEM(cormoran__paw3222, &paw3222_feature_meta, paw3222_rpc_handle_request);

ZMK_RPC_CUSTOM_SUBSYSTEM_RESPONSE_BUFFER(cormoran__paw3222, cormoran_paw3222_Response);

static void set_error(cormoran_paw3222_Response *resp, const char *fmt, ...) {
...
static bool paw3222_rpc_handle_request(const zmk_custom_CallRequest *raw_request,
    ...
    cormoran_paw3222_Response *resp =
        ZMK_RPC_CUSTOM_SUBSYSTEM_RESPONSE_BUFFER_ALLOCATE(cormoran__paw3222, encode_response);

    cormoran_paw3222_Request req = cormoran_paw3222_Request_init_zero;

    pb_istream_t req_stream =
        pb_istream_from_buffer(raw_request->payload.bytes, raw_request->payload.size);
    if (!pb_decode(&req_stream, cormoran_paw3222_Request_fields, &req)) {
    ...
    bool is_broadcast_get_info = req.which_request_type == cormoran_paw3222_Request_get_info_tag &&
